...
from models.psb.course_master_temp import CourseMasterTemp
...
def get_leave_students_filter():
    try:
        ...
        # Initial query on LeaveAttendanceTemp
        query = LeaveAttendanceTemp.query

        if student_name:
            search_pattern = f"%{student_name}%"
            query = query.filter(
                or_(
                    LeaveAttendanceTemp.student_name.ilike(search_pattern),
                    LeaveAttendanceTemp.student_id.ilike(search_pattern),
                )
            )
        ...
        if year and month:
            ...
            query = query.filter(
                LeaveAttendanceTemp.leave_date >= start_date,
                LeaveAttendanceTemp.leave_date <= end_date,
            )

        # Get all filtered leave records
        leave_records = query.all()

        if not leave_records:
            return jsonify({"students": [], "total_students": 0}), 200

        log.info("Total leave records count: %d", len(leave_records))

        response_list = []

        # Processing leave records without handling uniqueness
        for record in leave_records:
            # Fetching the student email from Attendance table
            attendance_record = Attendance.query.filter_by(
                student_id=record.student_id
            ).first()
            student_email = (
                attendance_record.student_email
            ...
            student_data = {
                ...
                "leave_reason": record.leave_reason or "No reason provided",
            ...
            # Append each student data to the response list
            response_list.append(student_data)
...
def student_percentage(attendance_records):
    ...
    # Calculate totals for Present, Late, and Leave
    total_present = sum(
        1 for record in attendance_records if record.attendance_status == "Present"
    )

    total_late = sum(
        1 for record in attendance_records if record.attendance_status == "Late"
    )

    # Count Absent and matching Leave records
    total_absent = sum(
        1 for record in attendance_records if record.attendance_status == "Absent"
    )

    absent_leave_count = 0
    for record in attendance_records:
        if record.attendance_status == "Absent":
            leave_record = LeaveAttendanceTemp.query.filter_by(
                student_id=record.student_id,
                leave_date=record.join_time,
                leave_status="Leave",
            ).first()
            if leave_record:
                absent_leave_count += 1

    total_leave = absent_leave_count

    # Calculate attendance percentage using the new formula
    total_attended_classes = total_present + total_late + total_leave
    percentage = (
        (total_attended_classes / total_classes) * 100 if total_classes > 0 else 0
    )

    return f"{max(min(int(percentage), 100), 0)}%"
