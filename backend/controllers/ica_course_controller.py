...
def get_courses_by_school_code(school_code):
    if not school_code:
        return make_response(
            jsonify({"message": "Please enter a school code"}), 400
        )
    try:
    ...
    try:
        ...
        if school_code:
            if not course_code:
                ...
                query = query.filter(
                    Attendance.course_code.in_(course_codes_list)
                )
            else:
        ...
        course_codes = {
            attendance.course_code for attendance in all_attendances
        }
        ...
        student_attendance_percentages = calculate_course_percentage(
            all_attendances
        )
...
def calculate_course_percentage(attendances):
    ...
    for student_id, data in student_attendance.items():
        ...
        overall_percentage = (
            (attended_modules / total_modules) * 100
            if total_modules > 0
            else 0
        )
...
def generate_attendance_report(
    ...
    if "enrollment_id" not in df.columns:
        raise ValueError(
            "Column 'enrollment_id' is missing from attendance data"
        )
    df = df[df["module_code"].isin(module_codes)]
    total_classes = df.groupby("enrollment_id").size()
    attended_classes = (
        df[df["status"].isin(["Present", "Late"])]
    ...
    report["attended_classes"] = (
        report["attended_classes"].fillna(0).astype(int)
    )
    report["percentage"] = (
