...
# def load_batchjob_data(job_type):
def load_batchjob_data(job_type):
    ...
    with current_app.app_context():
        try:
            ...
            construct_gantry_attendance_data(
                ...
                current_date,
                job_type,
                retry_date if retry_date else None,
                retry_type if retry_type else None,
            )
            ...
            return make_response(jsonify({"message": "Retry successful"}), 200)
...
def construct_gantry_attendance_data(
    ...
    try:
        ...
        if attendance_list_data and len(attendance_list_data) > 0:
            ...
            if not job_type == "retry":
                ...
                new_audit_log = BatchJob(
                    ...
                    + "Z",
                    batchjob_date=current_date.strftime(
                        "%Y-%m-%dT00:00:00.000Z"
                    ),
        ...
        #                 BatchJob.batchjob_date == retry_date,
        #                 BatchJob.batchjob_name.startswith(retry_type),
        #             )
        #             .order_by(BatchJob.created_at.desc())
        #             .first()
        #     )
        #     batch_results.failed_reports = failed_reports
        #     batch_results.batch_job_status="Failure"
        #     db.session.add(batch_results)
        #     db.session.commit()
        # else :
