...
def fetch_attendee_report(start_time, end_time):
    try:
        ...
        version = get_report_data.get("version")
        log.info(f"{version} is version")
        log.info(f"{int(version) + 1} is incremented version")
        ...
        log.info(f"{response.text} is response")
        ...
        log.info(f"{response_data} is response_data")
        csv_url = response_data.get("url")
        log.info(f"{csv_url} is csv_url")
        if not csv_url:
