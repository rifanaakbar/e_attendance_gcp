...
# ):
    ...
    data = request.json
    course_start_time_str= data.get("course_start_time_str")
    course_end_time_str= data.get("course_end_time_str")
    student_join_time_str= data.get("student_join_time_str")
    student_leave_time_str= data.get("student_leave_time_str")
    if not all(
        [
            course_start_time_str,
            course_end_time_str,
    ...
    try:
        course_start_time = parse_iso8601(str(course_start_time_str))
        course_end_time = parse_iso8601(str(course_end_time_str))
        student_join_time = parse_iso8601(str(student_join_time_str))
        student_leave_time = parse_iso8601(str(student_leave_time_str))
